...
const PrideTableRow = React.memo(({ pride, onDeleteClick }) => {
  return (
    <tr className="border-b hover:bg-gray-50 transition-colors">
      <td className="py-2 sm:py-3 px-3 sm:px-4 whitespace-nowrap">
        <div className="text-xs sm:text-sm text-gray-900">
          {moment(pride.updatedAt).format("MMM D, YYYY")}
        </div>
      </td>
      <td className="py-2 sm:py-3 px-3 sm:px-4 whitespace-nowrap">
        <Link to={`/pride/${pride.slug}`} className="flex items-center">
          {pride?.image ? (
            <img
              src={pride.image}
              alt={pride.name}
      ...
      <td className="py-2 sm:py-3 px-3 sm:px-4 whitespace-nowrap">
        <span className="text-xs sm:text-sm text-gray-600">{pride.award}</span>
      </td>
      <td className="py-2 sm:py-3 px-3 sm:px-4 whitespace-nowrap">
      ...
      <td className="py-2 sm:py-3 px-3 sm:px-4 whitespace-nowrap">
        <span className="text-xs sm:text-sm text-gray-600">{pride.year}</span>
      </td>
      <td className="py-2 sm:py-3 px-3 sm:px-4 whitespace-nowrap flex space-x-2">
        <Link
          to={`/Admin/CreatePride/${pride.slug}`}
        ...
        <button
          onClick={() => onDeleteClick(pride._id)}
...
export default function PrideInCategoryList() {
  const navigate = useNavigate();
  const [isLoading, setIsLoading] = useState(false);
  const [prideEntries, setPrideEntries] = useState([]);
  const [showMore, setShowMore] = useState(true);
  const [prideIdToDelete, setPrideIdToDelete] = useState("");
  ...
  const fetchPrideEntries = useCallback(
    async (
      startIndex = 0,
      search = "",
      category = "",
      ...
      setIsLoading(true);
      try {
        let url = `${backendURL}/api/getPrideEntries?startIndex=${startIndex}&limit=9`;
        ...
        if (res.ok) {
          const newPrideEntries =
            startIndex === 0
              ? data.prideEntries
              : [...currentPrideEntries, ...data.prideEntries];
          setPrideEntries(newPrideEntries);
          setShowMore(data.prideEntries.length === 9);
        } else {
          showAlertMessage(
            data.message || "Failed to fetch pride entries",
            "destructive"
          );
        }
      } catch (error) {
        showAlertMessage(
          error.message || "An error occurred while fetching pride entries",
          "destructive"
        );
      } finally {
        setIsLoading(false);
  ...
  const { categories, years } = useMemo(() => {
    ...
    const uniqueYears = [
      ...new Set(prideEntries.map((pride) => pride.year.toString())),
    ].sort((a, b) => b - a);
  ...
  const handleDeletePride = async () => {
    try {
      const res = await fetch(
        `${backendURL}/api/deletePride/${prideIdToDelete}`,
        {
          method: "DELETE",
        }
      );
      const data = await res.json();
      if (res.ok) {
        setPrideEntries((prev) =>
          prev.filter((pride) => pride._id !== prideIdToDelete)
        );
        showAlertMessage("Pride entry deleted successfully", "success");
        setDeleteOpen(false);
      } else {
        showAlertMessage(
          data.message || "Failed to delete pride entry",
          "destructive"
        );
      }
    } catch (error) {
      showAlertMessage(
        error.message || "An error occurred while deleting pride entry",
        "destructive"
      );
    }
  };

  const handleDeleteClick = (prideId) => {
    setPrideIdToDelete(prideId);
    setDeleteOpen(true);
  };
  ...
  if (isLoading && prideEntries.length === 0) {
    return (
    ...
    <div className="mx-auto px-3 sm:px-4 py-6 sm:py-8 max-w-7xl bg-accent-cream mt-12">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 sm:mb-6 gap-4">
        <h1 className="text-xl sm:text-2xl font-bold text-primary">
          Manage Pride Entries
        </h1>
        <button
          onClick={() => navigate("/Admin/CreatePride")}
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search pride entries..."
              value={searchTerm}
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-xs sm:text-sm text-gray-500">
              {searchTerm || filterCategory || filterYear
                ...
                : "Get started by creating a new pride entry"}
            </p>
            <div className="mt-6">
              <button
                onClick={() => navigate("/Admin/CreatePride")}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-primary mb-2">
              Are you sure you want to delete this pride entry?
            </h3>
