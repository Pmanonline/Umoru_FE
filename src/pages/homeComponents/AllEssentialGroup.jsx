...
const FilterButton = ({ text, count, isActive, onClick }) => (
  <button
    onClick={onClick}
    className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium hover:scale-105 transition-all duration-200 ${
      isActive
        ? "bg-blue-100 text-red-600"
        : "bg-gray-100 text-gray-600 hover:bg-gray-200"
    } flex items-center gap-1 sm:gap-2 m-1`}>
    <span>{text}</span>
    <span className="text-[10px] sm:text-xs bg-white px-1 py-0.5 rounded-full">
      {count}
    </span>
  </button>
);
...
const CompanyNamesButton = ({ title, url }) => (
  <Link target="_blank" to={url}>
    <button className="px-2 sm:px-4 py-1 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 bg-gray-100 text-gray-600 hover:bg-gray-200 hover:scale-105 flex items-center gap-1 sm:gap-2 m-1">
      <span>{title}</span>
    </button>
  </Link>
);
...
const HeroCard = ({ title, description, iconSrc, url }) => (
  <Card
    ...
    <div className="flex p-2 sm:p-4 items-start border-b border-gray-100">
      <img
        src={iconSrc}
        alt={title}
        className="w-10 h-10 sm:w-12 sm:h-12 object-cover rounded-md mr-2 sm:mr-4 flex-shrink-0"
      />
      <Typography
        component="div"
        className="font-black text-gray-800 text-sm sm:text-base break-words">
        {title}
      </Typography>
    </div>
    <CardContent className="bg-white flex-grow flex flex-col justify-between p-2 sm:p-4">
      <Typography
        variant="body2"
        className="text-gray-600 text-xs sm:text-sm mb-2 sm:mb-4">
      ...
      <Link target="_blank" to={url} className="no-underline">
        <div className="mt-1 sm:mt-2">
          <button className="px-0 text-red-600 font-bold rounded-md hover:scale-110 transition-all duration-200 flex items-center gap-1 sm:gap-2">
            <span>Explore</span>
            <svg
...
const AllEssentialGroup = () => {
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 py-8 sm:py-12">
      <h1 className="text-3xl sm:text-4xl md:text-5xl text-center font-semibold mb-4 sm:mb-6">
        The world is going{" "}
        <span className="text-NavClr italic font-bold">e</span> so are we
      </h1>
      <div className="mb-4 sm:mb-6 flex justify-center">
        <div className="relative w-full max-w-xs sm:max-w-md">
          ...
          />
          <SearchIcon className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4 sm:h-5 sm:w-5" />
        </div>
      </div>
      ...
      <div
        ...
        className="flex flex-col items-center mx-auto gap-2 mb-4 sm:mb-6 rounded-md p-2 sm:p-4 md:p-5 max-w-full sm:max-w-2xl md:max-w-3xl">
        <div className="w-full text-lg sm:text-xl text-white font-semibold text-center mb-2">
      ...
      {visibleCards < filteredServices.length && (
        <div className="flex justify-center mt-4 sm:mt-6">
          <button
            ...
            className="px-4 sm:px-6 py-2 bg-[#df6133] text-white rounded-full hover:bg-[#8b2323] transition-all duration-200 text-sm sm:text-base">
            See More
          </button>
        </div>
      )}
...
export default AllEssentialGroup;
